...
class GrowthBox:
    ...
    * ``"deck"`` — the region is the actual occupied volume of one or more parts
      in a **separate Radioss deck** (:attr:`region_rad`, resolved relative to
      ``model.case_dir`` like the load-case decks), the positive mirror of the
      keep-out deck (:attr:`Model.growth_keepout_rad`). Membership is centroid
      inside those parts' solid elements (exact point-in-tetrahedron, ``/BRICK``
      split into 6 tets), so an arbitrarily shaped region can be drawn in a
      pre-processor instead of approximated by primitives. :attr:`region_part_ids`
      selects which part ids form the region (empty = all solid parts) and
      :attr:`region_clearance_mm` grows the region outward by that gap (0 = the
      parts' volume exactly). The deck is **never solved** — only its geometry is
      read — so it needn't be a runnable model; it just defines *where* material
      may be added. Like every region it still selects **design elements** that
      must be pre-meshed there (or auto-generated by the growth-mesh PREPARE step,
      which meshes the deck region's bounding volume).
    ...
    """
    name: str = ""
    shape: str = "box"                # "box" | "cylinder" | "sphere" | "polyhedron" | "deck"
    ...
    # --- deck: the region is the occupied volume of parts in a separate deck ---
    # region_rad is a Radioss deck (relative to model.case_dir like the load-case
    # decks) whose parts' solid geometry defines the region -- the positive mirror
    # of model.growth_keepout_rad. region_part_ids selects which /TETRA4 (or
    # /BRICK) part ids form it (empty = all solid parts); region_clearance_mm grows
    # the region outward by that gap (0 = the parts' volume exactly).
    region_rad: Optional[str] = None
    region_part_ids: list = field(default_factory=list)
    region_clearance_mm: float = 0.0
